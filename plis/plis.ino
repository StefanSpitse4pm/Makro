...
unsigned long sinceStart = 0;
int startAmount = 0;
unsigned long previousMillis = 0;
int pulseWidth = GRIPPER_OPEN;
int exitCounterAsInteger = 0;
int sensorBlack = 0;
int mainAvg;
bool isOpen = true;
int guaranteedBlack;
  ...
  if (millis() - sinceSetup > 200) 
    {
        ...
        if (isOpen && currentMillis - sinceSetup >= 3000) 
        {
          pulseWidth = GRIPPER_CLOSE;  // Move to closed position
...
void exitCounter() 
{
    ...
    for (int i = 0; i < NUMSENSORS; i++) { 
      ...
      if(sensorReadings[i] >= guaranteedBlack) 
      {
        counter++;
      }
    }
    if (counter >= 8) 
    {
      exitCounterAsInteger++;
    }
    else 
    {
      exitCounterAsInteger = 0;
    }

    if (exitCounterAsInteger > 25) 
    {
      started = false;
      pulseWidth = GRIPPER_OPEN;
    }
...
void followLine() {
    ...
    mainAvg = sum / NUMSENSORS; 
    int deadzonehigh = mainAvg + 50;  
    ...
    if (leftTurnInProgress) {
        if (sensorReadings[3] >= deadzonehigh && sensorReadings[4] >= deadzonehigh && startedTurning >= ) {
