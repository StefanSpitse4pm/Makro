...
void followLine() {
  ...
  if (leftTurnInProgress) {
    if (sensorReadings[3] >= deadzonehigh && sensorReadings[4] >= deadzonehigh && startedTurning >= 25) {
  ...
  for (int i = 0; i < NUMSENSORS; i++) 
  {
    if (sensorReadings[i] >= deadzonehigh || leftTurnInProgress) 
    {
      break;
    }
    whiteSensors++;
